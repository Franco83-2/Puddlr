...
describe 'Feature Test: User Signout', :type => :feature do
  ...
  describe 'Feature Test: Go on a Puddle', :type => :feature do
  ...
  it 'has a link from the user show page to the puddle index page' do
    expect(page).to have_content("See Open Puddles")
    click_link('See Open Puddles')
  end
  ...
  it "when the user is too short, clicking on 'Go on this ride' displays a sorry message" do
    ...
    click_link("Go on #{@teacups.name}")
    click_button("Go on this ride")
    expect(page).to have_content("Sorry. You are not tall enough to ride the #{@teacups.name}")
